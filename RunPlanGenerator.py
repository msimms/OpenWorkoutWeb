...
class RunPlanGenerator(object):
    ...
    def gen_speed_run(self, speed_run_pace, easy_run_pace, exp_level):
        """Utility function for creating a speed workout."""

        # Set the number of reps based on the user's skill level.
        ...
        # Workout 0: One minute hard / One minute easy.
        selected_interval_workout_index = random.randint(0, 2)
        if selected_interval_workout_index == 0:
            interval_distance = RunPlanGenerator.nearest_interval_distance(1.0 * speed_run_pace)
            recovery_distance = interval_distance

        # Workout 1: 4x 1K repeats.
        elif selected_interval_workout_index == 1:
            interval_distance = 1000.0
            recovery_distance = 2.0 * speed_run_pace

        # Workout 2: 4x 1 mile repeats.
        elif selected_interval_workout_index == 2:
            interval_distance = 1609.344
            recovery_distance = 2.0 * speed_run_pace
        ...
        speed_run_workout.add_interval(interval_reps, interval_distance, speed_run_pace, recovery_distance, easy_run_pace)
        speed_run_workout.add_cooldown(10 * 60)
        return speed_run_workout

    def gen_track_interval_run(self, interval_run_pace, easy_run_pace, exp_level):
        """Utility function for creating a track interval workout."""

        # Build a collection of possible run interval sessions, sorted by target distance. Order is { reps, distance in meters }.
        interval_workouts = [ [6,100], [6,200], [6,300], [5,600], [4,800], [3,1000], [4,1000], [5,1000], [4,1600] ]
    ...
    def gen_workouts_for_next_week(self, inputs):
        ...
        if goal_type.lower() == Keys.GOAL_TYPE_SPEED.lower():
            ...
            interval_workout = self.gen_track_interval_run(interval_run_pace, easy_run_pace, exp_level)
            ...
            speed_workout = self.gen_speed_run(speed_run_pace, easy_run_pace, exp_level)
